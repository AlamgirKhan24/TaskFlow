...
function buildSeed() {
  ...
  add( 0, 'Mentor check-in',          '10:00 AM', 'meeting');
  add( 0, 'Submit Analytics task',    '',         'deadline');
  add( 2, 'SCSS code review',         '3:00 PM',  'meeting');
  add( 5, 'Evening walk',             '6:30 PM',  'personal');
  add(-1, 'Weekly standup',           '9:30 AM',  'meeting');
...
// viewDate tracks the month being displayed; only year+month are meaningful, day is always 1.
...
function render() {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const pills = visible.map((ev) => `<div class="event-pill ${ev.category}">${ev.title}</div>`).join('');
...
function renderSidePanel() {
  ...
  Object.values(events).forEach((list) =>
    list.forEach((ev) => { if (counts[ev.category] !== undefined) counts[ev.category]++; })
  );
  ...
  upcomingList.innerHTML = upcoming.slice(0, 6).map((ev) => {
    const label = sameDay(ev.dateObj, today)
      ? 'Today'
      : ev.dateObj.toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
    return `
...
function resetCategoryPicker() {
  ...
  categoryPicker.querySelectorAll('.cat-pill').forEach((p) =>
    p.classList.toggle('active', p.dataset.cat === 'work')
  );
}
  ...
  eventList.innerHTML = dayEvents.map((ev) => `
    <div class="day-event-row" data-id="${ev.id}">
      ...
      <div class="day-event-info">
        ...
        <div class="day-event-time">${ev.time ? `${ev.time} · ` : ''}${CATEGORY_LABEL[ev.category]}</div>
      </div>
